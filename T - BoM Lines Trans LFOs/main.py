...
def draw(screen, etc) :
    global top_screen, bottom_screen, trig, r, g, b, x, y, x2, y2, scalar, linelfo
    
    scalar2=scalar.update()
    scalar.step = etc.knob2/2 # trails screen transparency rate
    placex = int(1280 * scalar2)
    placey = int(720*scalar2)
    
    line = int(linelfo.update())
    linelfo.step = etc.knob1*30 # line width/length/placement change rate
    ...
    if trig == 1 :
        ...
        x2 = random.randrange(-10,abs(line*4)+1)
        y2 = random.randrange(-10,abs(line*3)+1)
        trig = 0 

    pygame.draw.line(screen,(r,g,b),[x,y],[x2,y2],line)
